...
class CheckTest {
    ...
    @Test
    void getMostCommonProductInPizzas_CertainString_GetStringOfMostCommonProduct(){
        pizza_arr = new ArrayList<>();
        Pizza pizza1 = new Pizza("шось", s, prods, b);
        Pizza pizza2 = new Pizza("шось 2", s, prods2, b);
        pizza2.addTopping(p);
        ...
        h1 = new Human("Василь","Петренко");
        Check check = new Check(pizza_arr, h1);
        assertEquals("кукуруза", check.getMostCommonProductInPizzas());
    }

    @Test
    void getTotalWeightOfProductsInPizzas_CertainWeight_GetSumOfWeights(){
        Check check = new Check(pizza_arr, h1);
        assertEquals(335, check.getTotalWeightOfProductsInPizzas());
    }
}